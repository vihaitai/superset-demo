...
RUN if [ "$DEV_MODE" = "false" ]; then \
        npm run ${BUILD_CMD} --registry=https://registry.npmmirror.com; \
    else \
...
RUN if [ "$DEV_MODE" = "false" ]; then \
        npm run build-translation --registry=https://registry.npmmirror.com; \
    else \
...
RUN --mount=type=cache,target=/root/.cache/pip \
    apt-get update -qq && apt-get install -yqq --no-install-recommends \
      build-essential \
    && pip install -r requirements/development.txt -i https://mirrors.aliyun.com/pypi/simple \
    && apt-get autoremove -yqq --purge build-essential \
    && rm -rf /var/lib/apt/lists/*
